...
(() => {
        ...
        if (!this._earnedAchievements.includes(id)) {
            ...
            if (data) {
                ...
                if (data.goldReward > 0) {
                    setTimeout(() => {
                        try {
                            const rewardPopup = new Window_RewardPopup(data.name, `+${data.goldReward} of Gold`);
    ...
    Game_System.prototype.updateAchievementProgress = function (type, value) {
        ...
        const progressEntry = ProgressiveAchievements.find(entry => entry.type === type);
        const goal = progressEntry ? Number(progressEntry.goal) : Infinity;

        const current = this._achievementProgress[type];
        if (current >= goal) return; // Jeśli już osiągnięto cel, nie zwiększaj

        this._achievementProgress[type] = Math.min(current + value, goal);

        if (this._achievementProgress[type] >= goal) {
            this.earnAchievement(progressEntry.achievementId);
        }
    };
    ...
    class Window_RewardPopup extends Window_Base {
        ...
        static calcPosition(width, height) {
            ...
            switch (rewardPopupPosition) {
                ...
                case "TopRight": return { x: xRight, y: yTop };
                case "BottomLeft": return { x: pad, y: yBottom };
                case "BottomCenter": return { x: xCenter + 150, y: yBottom + 20 };
                case "BottomRight":
                default: return { x: xRight, y: yBottom };
            }
        }
